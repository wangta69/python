...
def test1():
    ...
    low_pbr = MagicUtil.get_value_rank(MagicUtil.select_code_by_price(price_df, invest_df, start_date), 'PBR', strategy_date, 20)
    low_per = MagicUtil.get_value_rank(MagicUtil.select_code_by_price(price_df, invest_df, start_date), 'PER', strategy_date, 20)
    pbr_backtest = MagicUtil.backtest_beta(price_df, low_pbr, start_date, end_date, initial_money)
    per_backtest = MagicUtil.backtest_beta(price_df, low_per, start_date, end_date, initial_money)
...
def test2():
    ...
    #end_date = '2018-5'
    initial_money = 100000000
    all_pbr = MagicUtil.get_value_rank(MagicUtil.select_code_by_price(price_df, invest_df, start_date), 'PBR', strategy_date, None).dropna()
    length = int(len(all_pbr)/5)
    pbr_backtest1 = MagicUtil.backtest_beta(price_df, all_pbr[:length], start_date, end_date, initial_money)
    pbr_backtest2 = MagicUtil.backtest_beta(price_df, all_pbr[length:length*2], start_date, end_date, initial_money)
    pbr_backtest3 = MagicUtil.backtest_beta(price_df, all_pbr[length*2:length*3], start_date, end_date, initial_money)
    pbr_backtest4 = MagicUtil.backtest_beta(price_df, all_pbr[length*3:length*4], start_date, end_date, initial_money)
    pbr_backtest5 = MagicUtil.backtest_beta(price_df, all_pbr[length*4:], start_date, end_date, initial_money)
...
def test3():
    ...
    end_date = '2017-5'
    initial_money = 100000000
    f_score_result = MagicUtil.get_fscore(MagicUtil.select_code_by_price(price_df, fs_df, start_date), strategy_date, 20)
    f_score_backtest = MagicUtil.backtest_beta(price_df, f_score_result, start_date, end_date, initial_money)
...
def backtest_re(strategy, start_date, end_date, initial_money, price_df, fr_df, fs_df, num, value_type=None,
    ...
    for temp in range(start_year, end_year):
        ...
        if strategy.__name__ == 'high_roa':
            st_df = strategy(MagicUtil.select_code_by_price(price_df, fr_df, this_term_start), strategy_date, num)
        elif strategy.__name__ == 'magic_formula':
            st_df = strategy(sMagicUtil.elect_code_by_price(price_df, invest_df, this_term_start), strategy_date, num)
        elif strategy.__name__ == 'get_value_rank':
            st_df = strategy(MagicUtil.select_code_by_price(price_df, invest_df, this_term_start), value_type, strategy_date, num)
        elif strategy.__name__ == 'make_value_combo':
            st_df = strategy(value_list, MagicUtil.select_code_by_price(price_df, invest_df, this_term_start), strategy_date, num)
        elif strategy.__name__ == 'get_fscore':
            st_df = strategy(MagicUtil.select_code_by_price(price_df, fs_df, this_term_start), strategy_date, num)
            ...
            st_df = strategy(MagicUtil.select_code_by_price(price_df, invest_df, this_term_start),
                             MagicUtil.select_code_by_price(price_df, fs_df, this_term_start), strategy_date, num)

        backtest = MagicUtil.backtest_beta(price_df, st_df, this_term_start, this_term_end, initial_money)
...
end_date = '2018-5'
initial_money = 100000000
strategy = MagicUtil.get_value_rank
back_test_result1 = backtest_re(strategy, start_date, end_date, initial_money, price_df, fr_df, fs_df, 20, value_type='PER')
back_test_result2 = backtest_re(strategy, start_date, end_date, initial_money, price_df, fr_df, fs_df, 20, value_type='PBR')
