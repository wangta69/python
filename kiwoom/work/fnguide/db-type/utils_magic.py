...
class MagicUtil:
    ...
    def crawalSvdMain(self, code):
        ...
        # print(tables[7]) # 투자의견, 목표주가, EPS, PER, 추정기관수

        # print(tables[10]) # 매출액, 영업이익, 당기순이익, 지배주주순이익, 자본총계, 자본금, 부채비율, 유보율, ROA, ROE, EPS, BPS, DPS, PER, PBR, 발생주식수
        # df = tables[10]
        ...
        #     print('================================')
        #     print(idx, column)
        #     self.mysql.financeinfoFnguide1(code, idx, column)

        print('============================')
        # print(tables[0]) # 종가, 최고가, 수익률, 시가총액, 발생주식수(보통주 / 우선저)
        df = tables[0]
        stock_amt = df.iloc[6, 1];
